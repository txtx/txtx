...
The `evm::get_contract_from_foundry_project` and `evm::get_contract_from_hardhat_project` functions can be used to retrieve the contract object.
 This is an object type containing the keys:
  ...
  <Property name="linked_libraries" required="optional" type="object">
    A map of contract name to contract address to specify the linked libraries for the deployed contract. This is an object type containing the keys:
  - **contract_name**: A contract name (key) mapped to an address. If a contract deployment requires a linked library, this contract address will be used for all occurrences of the specified library name.
  </Property>

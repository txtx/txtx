...
fn extract_input_reference(content: &str, position: &Position) -> Option<String> {
    ...
    // Use AST-based extraction with lenient cursor detection
    let (hcl_ref, _range) = hcl_ast::extract_reference_at_position_lenient(content, *position)?;

    // Only return Input references
    match hcl_ref {
        hcl_ast::Reference::Input(name) => Some(name),
        _ => None,
    }
}

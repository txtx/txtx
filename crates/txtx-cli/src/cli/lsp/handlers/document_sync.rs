    ...
    pub fn did_save(&self, params: DidSaveTextDocumentParams) -> Option<PublishDiagnosticsParams> {
        ...
        let diagnostics = if document.is_runbook() {
            ...
            if let Some(manifest) = manifest {
                let diagnostics_by_file = crate::cli::lsp::diagnostics_multi_file::validate_with_multi_file_support(
                    uri,
                    document.content(),
                    Some(manifest),
                ...
                // Return diagnostics for the requested file only
                diagnostics_by_file.get(uri).cloned().unwrap_or_default()
            } else {

...
impl<'a> EnvironmentResolver<'a> {
    ...
    /// Returns all effective inputs for the current environment with their source.
    ///
    /// Implements txtx's environment inheritance model: environment-specific values
    /// override global defaults. Each input tracks its source environment for debugging.
    ///
    /// # Returns
    ///
    /// Map of input key to (value, source_environment)
    pub fn get_effective_inputs(&self) -> HashMap<String, (String, String)> {
        let mut effective_inputs = HashMap::new();

        // Global environment provides defaults
        if let Some(global_vars) = self.manifest.environments.get("global") {
    ...
    /// Returns whether a value is inherited from global environment rather than defined locally.

use txtx_test_utils::{
    RunbookBuilder, 
    builders::{ValidationMode, ValidationResult},
    create_test_manifest_with_env,
    assert_validation_error, assert_success
};
...
/// 2. Multi-mode validation (HCL-only vs Doctor)
...
fn basic_runbook_example() {
    ...
    let mut builder = RunbookBuilder::new()
        ...
        // Define a signer
        .signer("deployer", "evm::secp256k1", vec![
            ("private_key", "env.DEPLOYER_KEY")
        ])
        // Add a variable
...
fn environment_aware_runbook_example() {
    ...
    // Validate with doctor mode for full validation
    let result = builder.validate_with_doctor(Some(manifest), Some("development".to_string()));
    
    println!("Validation result for development environment: {}", 
...
fn multi_action_workflow_example() {
    ...
    let result = builder.validate();
    println!("Multi-action workflow validation: {}", 
        if result.success { "✓ Success" } else { "✗ Failed" });
    println!();
}
...
fn cross_chain_deployment_example() {
    ...
    let mut builder = RunbookBuilder::new()
        ...
        .action("setup_bridge", "mainnet::call")
            ...
            .input("args", "[action.deploy_optimism.contract_address, action.deploy_arbitrum.contract_address]")
            .input("depends_on", "[action.deploy_optimism, action.deploy_arbitrum]");
    
    let result = builder.validate();
    println!("Cross-chain deployment validation: {}", 
        if result.success { "✓ Success" } else { "✗ Failed" });
    println!();
}
...
fn validation_modes_example() {
    ...
    let runbook = || RunbookBuilder::new()
        ...
        .output("result", "action.test.invalid_field");  // Invalid field
    
    // Test 1: HCL-only validation
    let mut builder1 = runbook();
    let hcl_result = builder1.validate();
    println!("HCL-only validation: {}", 
        if hcl_result.success { "✓ Passed" } else { "✗ Failed" });
    if !hcl_result.errors.is_empty() {
        println!("  Errors detected: {}", hcl_result.errors.len());
    }
    
    // Test 2: Doctor validation (would catch more issues)
    let mut builder2 = runbook();
    let doctor_result = builder2.validate_with_mode(ValidationMode::Doctor {
        manifest: None,
        environment: None,
        file_path: Some(PathBuf::from("test.tx")),
    });
    println!("Doctor validation: {}", 
        if doctor_result.success { "✓ Passed" } else { "✗ Failed" });
    if !doctor_result.errors.is_empty() {
        println!("  Errors detected: {}", doctor_result.errors.len());
...
fn complex_defi_workflow_example() {
    ...
    let mut builder = RunbookBuilder::new()
        ...
        // Signers
        .signer("treasury", "evm::secp256k1", vec![
            ("private_key", "env.TREASURY_KEY")
        ])
        ...
        .action("deploy_token", "evm::deploy_contract")
            ...
            .input("constructor_args", "[variable.token_name, variable.token_symbol, variable.initial_supply]")
        ...
        .action("deploy_governor", "evm::deploy_contract")
            ...
            .input("constructor_args", "[action.deploy_token.contract_address, action.deploy_timelock.contract_address]")
            .input("signer", "signer.treasury")
    ...
    // Validate with doctor mode
    let result = builder.validate_with_doctor(Some(manifest), Some("production".to_string()));
    
    println!("Complex DeFi workflow validation: {}", 
        if result.success { "✓ Success" } else { "✗ Failed" });
...
mod tests {
    ...
    fn test_multi_file_support() {
        ...
        assert!(builder.files.contains_key("contracts/Token.sol"));
    }

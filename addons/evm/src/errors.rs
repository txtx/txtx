...
pub enum VerificationError {
    ...
    TransactionReverted { tx_hash: String, reason: Option<String> },
    LogDecodingFailed { tx_hash: String, error: String },
    InsufficientConfirmations { required: usize, current: usize },
}
...
impl fmt::Display for VerificationError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::TransactionNotFound { tx_hash } => write!(f, "Transaction {} not found", tx_hash),
            Self::TransactionReverted { tx_hash, reason } => {
            ...
            Self::LogDecodingFailed { tx_hash, error } => {
                write!(f, "Failed to decode logs for transaction {}: {}", tx_hash, error)
            }
            Self::InsufficientConfirmations { required, current } => {
                write!(f, "Insufficient confirmations: {} required, {} current", required, current)
            }
        }
    }
}
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::InvalidType { expected, received } => {
                write!(f, "Type mismatch: expected type '{}', but received type '{}'", expected, received)
            }
            Self::FunctionNotFound { name } => {
                write!(f, "Function '{}' not found in ABI", name)
            }
            Self::ConstructorNotFound => write!(f, "Constructor not found in ABI"),
            Self::ArgumentCountMismatch { expected, got } => {
                write!(f, "Argument count mismatch: expected {} argument{}, got {}", 
                    expected, 
                    if *expected == 1 { "" } else { "s" },
                    got)
            }
            Self::InvalidArrayLength { expected, got } => {
                write!(f, "Invalid array length: expected array of length {}, got {}", expected, got)
            }
            Self::ArrayDimensionMismatch => write!(f, "Array dimension mismatch"),
            Self::UnsupportedAbiType(ty) => write!(f, "Unsupported ABI type: {}", ty),
            Self::TypeSpecifierParseFailed(msg) => {
                write!(f, "Failed to parse type specifier: {}", msg)
            }
            Self::InvalidValue { value_type, target_type } => {
                write!(f, "Cannot convert {} to {}", value_type, target_type)
            }

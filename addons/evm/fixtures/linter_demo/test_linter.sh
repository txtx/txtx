...
echo "=== Testing txtx lint command ==="
echo ""

# Create a simple test case
mkdir -p /tmp/lint_test/runbooks

cat > /tmp/lint_test/txtx.yml << 'EOF'
name: test_project
description: Test project for lint command
...
cat > /tmp/lint_test/runbooks/transfer.tx << 'EOF'
addon "evm" {
    chain_id = "11155111"
...
# These will be flagged by lint - send_eth only outputs tx_hash!
output "sender" {
...
echo "Created test files in /tmp/lint_test"
echo ""

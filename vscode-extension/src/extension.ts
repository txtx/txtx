...
// Global client reference for deactivate
...
function createMiddleware(outputChannel: vscode.OutputChannel): LanguageClientOptions['middleware'] {
  ...
  return {
    ...
    provideRenameEdits: async (document, position, newName, token, next) => {
      logRequest('Rename', document, position);
      outputChannel.appendLine(`[Rename] New name: ${newName}`);
      try {
        const result = await next(document, position, newName, token);
        logResponse('Rename', result);
        return result;
      } catch (error) {
        logError('Rename', error);
        throw error;
      }
    },
    prepareRename: async (document, position, token, next) => {
      logRequest('PrepareRename', document, position);
      try {
        const result = await next(document, position, token);
        logResponse('PrepareRename', result);
        return result;
      } catch (error) {
        logError('PrepareRename', error);
        throw error;
      }
    }
  };

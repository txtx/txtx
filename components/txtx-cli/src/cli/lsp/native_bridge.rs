use super::clarity_diagnostics_to_tower_lsp_type;
...
use txtx_core::kit::channel::{
    Receiver as MultiplexableReceiver, Select, Sender as MultiplexableSender,
};
use txtx_lsp::backend::{
...
impl LanguageServer for LspNativeBridge {
    ...
    async fn did_save(&self, params: DidSaveTextDocumentParams) {
        ...
        if let Some(contract_location) = utils::get_runbook_location(&params.text_document.uri) {
            let _ = match self.notification_tx.lock() {
                Ok(tx) => tx.send(LspNotification::RunbookSaved(contract_location)),
                Err(_) => return,
            };
        } else if let Some(manifest_location) =
            utils::get_manifest_location(&params.text_document.uri)
    ...
    async fn did_change(&self, params: DidChangeTextDocumentParams) {
        ...
        if let Some(contract_location) = utils::get_runbook_location(&params.text_document.uri) {
            if let Ok(tx) = self.notification_tx.lock() {
                let _ = tx.send(LspNotification::RunbookChanged(
                    contract_location,
    ...
    async fn did_close(&self, params: DidCloseTextDocumentParams) {
        if let Some(contract_location) = utils::get_runbook_location(&params.text_document.uri) {
            if let Ok(tx) = self.notification_tx.lock() {
                let _ = tx.send(LspNotification::RunbookClosed(contract_location));
            };
        }
    }
}

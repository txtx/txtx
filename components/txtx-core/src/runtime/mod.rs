...
use daggy::{RecursiveWalk, Walker};
use txtx_addon_kit::types::ConstructUuid;

pub fn run(manual: &Manual, addons_ctx: &AddonsContext) -> Result<(), String> {
    ...
    let mut walker = manual.constructs_graph.recursive_walk(root, |g, n| {
        g.children(n).iter(g).find(|&(e, n)| true)
    });

    while let Some((_edge, node)) = walker.walk_next(&manual.constructs_graph) {
        let uuid = manual.constructs_graph
            .node_weight(node)
